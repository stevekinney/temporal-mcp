...
This section defines a concrete 6-agent execution model for `R1` through `R4`.

## Agent Allocation (6 Agents)

| Agent | Charter | Primary Roadmap Coverage | Primary Paths |
| --- | --- | --- | --- |
| `A1 Protocol` | Own MCP protocol correctness and capability plumbing | MCP compliance baseline, tasks/elicitation/roots plumbing, completions spec track, subscriptions | `packages/server/src/mcp/**`, `packages/server/src/transports/**` |
| `A2 Temporal SDK` | Own SDK-first workflow/schedule/activity tooling | connectivity, read tools, safe-write SDK-backed tools | `packages/temporal/src/connect.ts`, `packages/temporal/src/read.ts`, `packages/temporal/src/write.ts` |
| `A3 Raw gRPC + Cloud` | Own non-wrapper API surface and profile routing | capability matrix enforcement, raw `workflowService`/`operatorService`, `@temporalio/cloud` paths | `packages/temporal/src/grpc.ts`, `packages/temporal/src/cloud.ts`, `packages/temporal/src/capability-matrix.ts` |
| `A4 Docs + Retrieval` | Own docs corpus, indexing, and docs-facing resources/tools | docs sync, chunking, search, docs resources | `packages/docs/src/**`, `packages/server/src/mcp/resources.docs.ts` |
| `A5 Policy + Security` | Own safety controls and sensitive-path hardening | policy engine, redaction, idempotency, dual-confirm framework, audit logging | `packages/server/src/policy/**`, `packages/server/src/observability/**`, `packages/server/src/security/**` |
| `A6 CI + Release` | Own test matrix, generation pipelines, and release automation | conformance suites, integration tests, docs generation checks, packaging/release | `.github/workflows/**`, `scripts/**`, root `package.json`, `documentation/**` |

## Work Allocation by Release Gate

| Release Gate | Lead Agents | Supporting Agents | Exit Artifact |
| --- | --- | --- | --- |
| `R1 Foundation` | `A1`, `A2`, `A4`, `A5` | `A3`, `A6` | Protocol baseline + config/policy + docs + read-only temporal slice integrated |
| `R2 GA` | `A2`, `A5`, `A6` | `A1`, `A3`, `A4` | Safe-write GA with idempotency/cancellation/progress and passing CI gates |
| `R3 Post-GA` | `A3`, `A5` | `A1`, `A2`, `A6` | Destructive/admin + batch + escape hatches with dual-confirm and audit guarantees |
| `R4 Remote` | `A1`, `A6` | `A3`, `A5` | Streamable HTTP mode with explicit auth strategy and parity validation |

## Parallelization Rules

1. `A1`, `A5`, and `A6` must complete `N1` to unblock all domain-tool coding.
2. `A2` and `A4` can execute in parallel after `N1`, but both are blocked on shared contract artifacts from `N2`.
3. `A3` starts raw gRPC/cloud work after `N3` capability matrix freeze and must not bypass `A5` policy wrappers.
4. No agent merges tool handlers until contract tests generated by `A6` pass for that tool family.
5. `R3` and `R4` work may proceed on dedicated branches but cannot merge before `R2` GA tag.

## Dependency Graph (Concrete DAG)

Nodes:

- `N1` MCP baseline complete (`initialize`, negotiation, cancellation/progress/pagination/logging)
- `N2` Tool contract + registry metadata frozen
- `N3` Capability matrix + profile routing rules finalized
- `N4` Config system with `mcp`, `transport`, `security`, `profile.kind`
- `N5` Policy/security wrappers and structured error envelope
- `N6` Tasks + Elicitation + Roots integration
- `N7` Docs subsystem
- `N8` Temporal connectivity + read-only tools (SDK-first)
- `N9` Raw gRPC/cloud fallback implementations
- `N10` Resources/templates + subscriptions
- `N11` Safe-write GA tools with idempotency/cancellation semantics
- `N12` Reliability/performance hardening
- `N13` CI/release/docs generation gates
- `N14` GA readiness verification
...
3. Test evidence (unit/integration/conformance).
4. Open risks and blocked dependencies.
5. Exact next node(s) in the dependency graph.

## Merge and Integration Policy

1. One integration branch per release gate: `integration/r1`, `integration/r2`, `integration/r3`, `integration/r4`.
2. Agents develop on short-lived branches prefixed with `codex/a<id>/...`.
3. Merge order follows DAG topological order; DAG violations are rejected.
4. `A6` owns merge queue policy and release cut criteria.
